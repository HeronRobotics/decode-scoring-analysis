...
function Timeline({ events = [], currentTime = 0, mode = null }) {
  ...
  const phaseRegions = useMemo(() => {
    ...
    return [
      { start: 0, end: autoEnd, label: "AUTO", color: "bg-brand-muted/20" },
      { start: autoEnd, end: bufferEnd, label: "BUFFER", color: "bg-brand-muted/60" },
      { start: bufferEnd, end: teleopEnd, label: "TELEOP", color: "bg-brand-muted/20" },
    ];
  }, [mode]);
  ...
  return (
    <div className="timeline-container w-full">
      ...
      {mode === "match" && !isExpanded && phaseScoreSummary && (
        <div className="px-4 py-4 bg-brand-surface">
          <div className="flex items-stretch gap-2">
            <button
              type="button"
              onClick={() => expandAndScrollTo(phaseScoreSummary.auto.start)}
              className="flex-1 flex flex-row items-center justify-between rounded-lg border border-brand-border bg-brand-accentBg/60 hover:bg-brand-accentBg transition-colors px-3 py-3 text-left"
            ...
            >
              ...
              <div className="">
                ...
                <div className="mt-0.5 text-[11px] text-right text-brand-text">
            ...
            <button
              ...
              className="flex-[0.35] rounded-lg border border-brand-border bg-brand-bg hover:bg-brand-accentBg/40 transition-colors px-3 py-3 text-left"
              title="Click to expand timeline"
            >
              <div className="text-xs font-bold tracking-wider text-brand-accent">
                BUFFER
            ...
            <button
              ...
              className="flex-[1.6] flex flex-row items-center justify-between rounded-lg border border-brand-border bg-brand-accentBg/60 hover:bg-brand-accentBg transition-colors px-3 py-3 text-left"
              title="Click to expand timeline"
            >
              <div className="text-xs font-bold tracking-wider text-brand-accent">
                TELEOP
              ...
              <div className="">
                ...
                <div className="mt-0.5 text-[11px] text-right text-brand-text">
                  38–{matchRecorderConstants.MATCH_TOTAL_DURATION}s
                </div>
              </div>
            </button>
          </div>
      ...
      {isExpanded && (
        <div ref={scrollRef} className="timeline-scroll bg-brand-surface">
          <div
          ...
          >
            ...
            {timeMarkers.map((time) => (
              <div
                key={time}
                className="absolute top-0 bottom-0 w-px bg-brand-muted"
                ...
                if (event.type === "cycle") {
                    ...
                    balls.push(
                      <Basketball
                        ...
                        className={`timeline-ball-enter ${
                          isScored ? "text-brand-success" : "text-brand-muted"
                        }`}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`w-0.5 h-2 ${
                          colorClass === "success" ? "bg-brand-success" : "bg-brand-muted"
                        }`}
                      />
                    </div>
                  );
            ...
            <div
              className="absolute top-0 bottom-8 w-0.5 bg-brand-accent z-10 transition-all duration-100"
            ...
            <div className="absolute bottom-0 left-0 right-0 h-8 bg-brand-surface border-t border-brand-border flex items-center">
              {timeMarkers.map((time) => (
                <div
                  key={time}
